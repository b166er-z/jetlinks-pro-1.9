...
import org.jetlinks.pro.network.mqtt.server.*;
...
class VertxMqttConnection implements MqttConnection {
    ...
    public VertxMqttConnection(NetMonitor netMonitor,
        ...
        this.endpoint
            ...
            .publishHandler(msg -> {
                ...
                if (SystemUtils.memoryIsOutOfWatermark()) {
                    //不使用log.error,防止内存进一步崩溃
                    System.err.println(
                        "JVM可用内存不足,丢弃MQTT[" + endpoint.clientIdentifier() + "]推送消息[" + msg.topicName() + "]."
                    );
                    return;
                }
                ...
                netMonitor.bytesRead(msg.payload().length());
                if (hasDownstream) {
    ...
    public InetSocketAddress getClientAddress() {
        if (clientAddress == null && endpoint.isConnected()) {
        ...
        return Mono
            .<Void>create(sink -> {
                ...
                endpoint.publish(message.getTopic(),
                                 ...
                                 result -> {
                                     if (result.succeeded()) {
                                         netMonitor.bytesSent(buffer.length());
                                         sink.success();
                                     } else {
