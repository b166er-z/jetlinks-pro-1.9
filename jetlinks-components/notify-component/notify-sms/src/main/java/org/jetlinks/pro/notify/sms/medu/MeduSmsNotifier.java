...
@Slf4j
public class MeduSmsNotifier extends AbstractNotifier<MeduSmsTemplate> {
    ...
    public MeduSmsNotifier(NotifierProperties profile,TemplateManager templateManager) {
    ...
    public MeduSmsNotifier(IClientProfile profile,TemplateManager templateManager) {
        this(new DefaultAcsClient(profile), templateManager);
    }

    public MeduSmsNotifier(IAcsClient client,TemplateManager templateManager) {
        super(templateManager);
        this.client = client;
    }
    ...
    @Override
    @Nonnull
    public Mono<Void> send(@Nonnull MeduSmsTemplate template, @Nonnull Values context) {
        ...
        return Mono.<Void>defer(() -> {
            try {
                ...
                request.putQueryParameter("PhoneNumbers", template.getPhoneNumber(ctx));
