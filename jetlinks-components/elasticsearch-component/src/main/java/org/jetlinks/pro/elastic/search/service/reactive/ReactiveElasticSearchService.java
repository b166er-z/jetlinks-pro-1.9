package org.jetlinks.pro.elastic.search.service.reactive;
...
import org.jetlinks.pro.elastic.search.utils.ElasticSearchConverter;
import org.jetlinks.pro.elastic.search.utils.QueryParamTranslator;
...
public class ReactiveElasticSearchService implements ElasticSearchService {
    ...
    private boolean checkWritable(String index) {
        if (SystemUtils.memoryIsOutOfWatermark()) {
            System.err.println("JVM内存不足,elasticsearch无法处理更多索引[" + index + "]请求!");
            return false;
        }
        return true;
    }
