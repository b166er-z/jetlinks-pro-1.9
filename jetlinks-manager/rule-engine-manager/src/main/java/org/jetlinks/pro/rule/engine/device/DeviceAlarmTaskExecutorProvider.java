...
//            List<Subscription> subscriptions = topics.stream().map(Subscription::new).collect(Collectors.toList());
            ...
            return resultFlux
                .flatMap(map -> {
                    ...
                    return createTopic(headers,
                                       String.format(
                                           "/rule-engine/device/alarm/%s/%s/%s",
                                           rule.getProductId(),
                                           map.get("deviceId"),
                                           rule.getId()))
                        .flatMap(topic -> eventBus.publish(topic, map))
                        .then(Mono.just(map));
    ...
    @SuppressWarnings("all")
    public Flux<String> createTopic(Map<String, Object> headers, String topic) {
        if (headers == null) {
        ...
        PropertyConstants
            .getFromMap(PropertyConstants.tenantId, headers)
            .ifPresent(ids -> {
                topics.addAll(Topics.tenants(ids, topic));
            });

        PropertyConstants
            .getFromMap(PropertyConstants.groupId, headers)
            .ifPresent(ids -> {
                topics.addAll(Topics.deviceGroups(ids, topic));
            });

        PropertyConstants
            .getFromMap(PropertyConstants.tenantMemberId, headers)
            .ifPresent(ids -> {
                topics.addAll(Topics.tenantMembers(ids, topic));
            });
