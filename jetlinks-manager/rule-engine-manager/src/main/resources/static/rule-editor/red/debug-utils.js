...
RED.debug = (function () {
    ...
    function init(_config) {
        ...
        RED.comms.subscribe(id + "/*/event/error", function (topic, msg) {
            var data = {msg: msg.data.stack,type:"log",level:'error'};
            console.error(msg.data.stack)
            var node = RED.nodes.node(msg.headers.sourceNode);
            if (node) {
                data._source = {
                    id: node.id,
                    topic:msg.data.type,
        ...
        RED.comms.subscribe(id + "/*/logger/*", function (topic, msg) {
            var data = {msg: msg.message,type:"log",level:msg.level};
        ...
        RED.comms.subscribe(id + "/*/event/result", function (topic, msg) {
    ...
    function showMessageMenu(button, dbgMessage, sourceId) {
        ...
        if (!menuOptionMenu) {
            menuOptionMenu = RED.menu.init({
                id: "red-ui-debug-msg-option-menu",
                options: [
                    {
                        id: "red-ui-debug-msg-menu-item-collapse",
                    ...
                    {
                        id: "red-ui-debug-msg-menu-item-clear-pins",
                    ...
                    null,
                    {
                        id: "red-ui-debug-msg-menu-item-filter",
                    ...
                    {
                        id: "red-ui-debug-msg-menu-item-clear-filter",
    ...
    function handleDebugMessage(o) {
        ...
        if (!busy && (stack.length > 0)) {
            ...
            if (stack.length > numMessages) {
                stack = stack.splice(-numMessages);
            }
        }
    }
    ...
    function processDebugMessage(o) {
        ...
        if (sourceNode) {
            $('<a>', {
                href: "#",
                class: "red-ui-debug-msg-name"
            }).text('节点: ' + (o.name || sourceNode.name || sourceNode.id))
                .appendTo(metaRow)
        ...
        if (o.hasOwnProperty("type") && (o.type === "log") && o.hasOwnProperty('level')) {
            ...
            //     errorLvlType = 'warn';
            // }
            msg.addClass('red-ui-debug-msg-level-' + o.level);
            $('<span class="red-ui-debug-msg-topic">log : (' + o.level + ')</span>').appendTo(metaRow);
        } else {
            ...
            $('<span class="red-ui-debug-msg-topic">' +
                (o.topic ? topic + ' : ' : '') +
                (o.property ? 'data.' + property : 'data') + " : " + format +
