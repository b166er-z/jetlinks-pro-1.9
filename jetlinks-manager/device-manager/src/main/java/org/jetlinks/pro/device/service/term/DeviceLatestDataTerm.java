...
import org.jetlinks.pro.device.service.data.DatabaseDeviceLatestDataService;
...
public class DeviceLatestDataTerm extends AbstractTermFragmentBuilder {
    ...
    Map<String, RDBTableMetadata> tableCache = Caches.newCache();
    ...
    public SqlFragments createFragments(String columnFullName, RDBColumnMetadata column, Term term) {
        ...
        RDBTableMetadata metadata = tableCache.computeIfAbsent(tableName, name -> column.getOwner().getSchema().getTable(name).orElse(null));
        ...
        sqlFragments.addSql("exists(select 1 from ", metadata.getFullName(), " _tmp where _tmp.id =", columnFullName);
