...
public class DeviceDetail {
    ...
    private List<DeviceBindInfo> binds = new ArrayList<>();
     ...
     * @since 1.7
     */
    public DeviceDetail withBind(List<DeviceBindEntity> binds) {
        if (binds == null) {
            return this;
        }
        for (DeviceBindEntity bind : binds) {
            this.binds.add(
                DeviceBindInfo.of(
                    bind.getId(),
                    bind.getType(),
                    DeviceBindHolder.lookup(bind.getType()).map(DeviceBindProvider::getName).orElse(bind.getType()),
                    bind.getKey(),
                    bind.getDescription()
                )
            );
    ...
    public Mono<DeviceDetail> with(DeviceOperator operator, List<ConfigPropertyMetadata> configs) {
        return Mono
            ...
            .doOnNext(tp -> {
                ...
                setAddress(tp.getT1());
                setConnectServerId(tp.getT5());
                with(tp.getT4()
                       .getTags()
                       .stream()
                       .map(DeviceTagEntity::of)
                       .collect(Collectors.toList()));
                Map<String, Object> cachedConfigs = tp.getT6().getAllValues();
