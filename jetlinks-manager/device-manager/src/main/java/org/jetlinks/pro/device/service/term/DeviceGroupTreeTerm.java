...
/**
 * 根据设备分组及其子分组查询设备或者与设备关联的数据,如: 查询某个分组下的设备列表.
 * <p>
 * <b>
 ...
 */
@Component
public class DeviceGroupTreeTerm extends AbstractTermFragmentBuilder {

    public static final String termType = "dev-group-tree";

    public DeviceGroupTreeTerm() {
        super(termType, "按设备分组树查询设备");
    }
    ...
    public SqlFragments createFragments(String columnFullName, RDBColumnMetadata column, Term term) {
        ...
        sqlFragments.addSql(
            "and exists (select 1 from dev_device_group g where g.id = _bind.group_id and ",
            idList
                .stream()
                .map(r -> "path like (select concat(path,'%') from dev_device_group g2 where g2.id = ?)")
