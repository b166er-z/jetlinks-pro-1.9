...
@Getter
public enum AggType implements EnumDict<String> {
    ...
    range("极差值", flux -> org.jetlinks.pro.utils.math.MathFlux
        .rangeNumber(flux.map(CastUtils::castNumber), Function.identity())
        .flux()
    ),
    median("中位数", flux -> org.jetlinks.pro.utils.math.MathFlux
        .median(flux, val -> CastUtils.castNumber(val).doubleValue())
        .flux()),

    geometricMean("几何平均值", flux -> org.jetlinks.pro.utils.math.MathFlux
        .geometricMean(flux, val -> CastUtils.castNumber(val).doubleValue())
        .flux()),

    mean("算术平均值", flux -> org.jetlinks.pro.utils.math.MathFlux
        .mean(flux, val -> CastUtils.castNumber(val).doubleValue())
        .flux()),

    variance("方差", flux -> org.jetlinks.pro.utils.math.MathFlux
        .variance(flux, val -> CastUtils.castNumber(val).doubleValue())
        .flux()),

    dev("标准差", flux -> org.jetlinks.pro.utils.math.MathFlux
        .standardDeviation(flux, val -> CastUtils.castNumber(val).doubleValue())
        .flux()),

    slope("倾斜率", flux -> org.jetlinks.pro.utils.math.MathFlux
