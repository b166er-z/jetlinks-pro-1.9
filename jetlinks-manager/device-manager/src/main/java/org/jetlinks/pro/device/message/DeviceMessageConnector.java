...
public class DeviceMessageConnector implements DecodedClientMessageHandler {
    ...
    private Mono<Message> handleMessage(Message message) {
        //合并以前的属性数据
        if (message instanceof DeviceMessage && message.getHeader(Headers.mergeLatest).orElse(false)) {
            DeviceMessage msg = ((DeviceMessage) message);
            String productId = message.getHeader(PropertyConstants.productId).orElse("null");
            return Mono
                .zip(
                    Mono.justOrEmpty(DeviceMessageUtils.tryGetProperties(msg)),
                    registry.getDevice(msg.getDeviceId()).flatMap(DeviceOperator::getMetadata),
        ...
        if (message instanceof DeviceMessage) {
            ...
            return deviceRegistry
                ...
                .flatMapIterable(configs -> {
                    ...
                    String topic = createDeviceMessageTopic(productId, deviceMessage.getDeviceId(), deviceMessage);

                    List<String> groupIds = configs
                        .getValue(PropertyConstants.groupId)
                        .orElse(Collections.emptyList());

                    List<String> tenantIds = configs
                        .getValue(PropertyConstants.tenantId)
                        .orElse(Collections.emptyList());

                    List<String> tenantMembers = configs
                        .getValue(PropertyConstants.tenantMemberId)
                        .orElse(Collections.emptyList());

                    List<String> topics = new ArrayList<>(groupIds.size() + tenantIds.size() + tenantMembers.size() + 2);
                    topics.add(topic);
                    //机构
                    String orgId = configs.getValue(PropertyConstants.orgId).orElse(null);
                    if (StringUtils.hasText(orgId)) {
                        topics.add(Topics.org(orgId, topic));
                    }
                    if (!groupIds.isEmpty()) {
                        //分组
                        topics.addAll(Topics.deviceGroups(groupIds, topic));
                    }
                    if (!tenantIds.isEmpty()) {
                        //租户
                        topics.addAll(Topics.tenants(tenantIds, topic));
                    }
                    if (!tenantMembers.isEmpty()) {
                        //成员
                        topics.addAll(Topics.tenantMembers(tenantMembers, topic));
                    }
                    return topics;
                });
        }
    ...
    static final FastThreadLocal<StringBuilder> STRING_BUILDER_CACHE = new FastThreadLocal<StringBuilder>() {
        @Override
